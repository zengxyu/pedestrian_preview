...
from visualize_utilities.bar_grah import bar_graph, bar_graph_with_error, bar_line_graph
...
def test_compare_success_rate_of_different_algorithms_in_different_envs():
    ...
    conditions = {
        ...
        "folder_name": ["CATAttentionNetTemporalSpacialLSTM4_uncomfortable_distance",
                        "temporal_fix_freq_0.8",
                        "temporal_fix_freq_0.6",
                        "temporal_fix_freq_0.4"
                        ],
    ...
    x_names = ["Fix target", "0.8s", "0.6s", "0.4s"]
    ...
    x_label = "Inference every $s$ seconds"
    save_path = os.path.join(plot_folder, "success_rate_of_different_fix_freq.png")
    ...
    bar_line_graph(x_names, label_names, values, colors, title, x_label=x_label, y_label=y_label, width=8,
                   save_path=save_path)
...
def test_compare_path_length_of_different_algorithms_in_different_envs():
    ...
    colors = [COLORS[i] for i in color_index]
    assert len(colors) == len(label_names)
    y_label = "Path length (m)"
    x_label = "Inference every $s$ seconds"
    save_path = os.path.join(plot_folder, "path_length_of_different_of_different_fix_freq.png")
...
def test_compare_closest_distance_of_different_algorithms_in_different_envs():
    ...
    x_label = "Inference every $s$ seconds"
    save_path = os.path.join(plot_folder, "min_distance_of_different_of_different_fix_freq.png")

    title = "Compare different fix freq in different environments"
    # bar_graph(x_names, label_names, values, colors, title, y_label, save_path)
    bar_line_graph(x_names, label_names, values, colors, title, x_label=x_label, y_label=y_label, width=8,
                   save_path=save_path)
