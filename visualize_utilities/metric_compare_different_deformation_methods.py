...
def test_compare_success_rate_of_different_algorithms_in_different_envs():
    ...
    title = "Compare different deformation methods in different environments"
    bar_graph(x_names, label_names, values, colors, title, y_label, total_width=0.7, save_path=save_path)
...
def test_compare_path_length_of_different_algorithms_in_different_envs():
    ...
    save_path = os.path.join(plot_folder, "path_length_of_different_deformation_methods.png")
...
def test_compare_closest_distance_of_different_deformation_methods_in_different_envs():
    # 比较同一个方法在不同的环境配置下，如不同的人数，不同的行人速度下，度量的变化
    project_path = get_project_path()
    plot_folder = "plot"
    if not os.path.exists(plot_folder):
    ...
    save_path = os.path.join(plot_folder, "min_distance_mean_of_different_deformation_methods.png")
