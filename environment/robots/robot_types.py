...
def init_robot(p, client_id, robot_name, physical_step_duration, robot_config, sensor_config, start, yaw):
    if robot_name == RobotTypes.RaceCar:
        robot = DifferentialRaceCar(p, client_id, physical_step_duration, robot_config,
                                    sensor_config, start, yaw)
    elif robot_name == RobotTypes.Turtlebot:
        robot = TurtleBot(p, client_id, physical_step_duration, robot_config,
                          sensor_config, start, yaw)
    elif robot_name == RobotTypes.ObjectRobot:
        robot = ObjectRobot(p, client_id, physical_step_duration, robot_config,
                            sensor_config, start, yaw)
    else:
