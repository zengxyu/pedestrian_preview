...
from environment.gen_scene.gen_office_map import create_office_map, create_office_map2
...
def generate_env(num_starts, world_name):
    ...
    if world_name == "office":
        occupancy_map = create_office_map2(world_config)
...
def generate_n_envs():
    ...
    phase = "train"
    # 生成的场景保存位置
    parent_folder = "office_evacuation"
    envs_folder = os.path.join(get_project_path(), "data", parent_folder, phase, "envs")
    envs_images_folder = os.path.join(get_project_path(), "data", parent_folder, phase, "envs_images")
    envs_folder2 = os.path.join(get_project_path(), "data", parent_folder, phase, "envs2")
    ...
    if not os.path.exists(envs_images_folder2):
        os.makedirs(envs_images_folder2)
    ...
    while i < len(indexes):
        ...
        if os.path.exists(save_path):
            ...
            continue
        # 保存的位置
        save_path2 = os.path.join(envs_folder2, save_file_name)
        image_save_path2 = os.path.join(envs_images_folder2, image_save_file_name)
