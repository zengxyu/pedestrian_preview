...
def prm_path_planning(occ_map, num_samples, start, end):
    ...
    prm = PRMController(num_samples, obstacles, start, end, max(occ_map.shape))
    # Initial random seed to try
    initial_random_seed = 0
    path = prm.run_prm(initial_random_seed)
    # prm.get_shortest_path()
    return path
...
if __name__ == '__main__':
    ...
    plt.show()
    [start, end], sample_success = sg_in_distance_sampler(occupancy_map=dilate_image(occupancy_map, 2), distance_ratio=0.75)
